...
function renderTable(data) {
    ...
    <td>${item.timeIn || '-'}</td>
    <td>${item.timeOut || '-'}</td>
    <td><span class="status-badge ${item.status.toLowerCase()}">${item.status.charAt(0).toUpperCase() + item.status.slice(1)}</span></td>
    <td>
        <button onclick="editAttendance('${item.id}')" class="edit-btn">Edit</button>
    </td>
`;
            tableBody.appendChild(row);
        });
...
document.getElementById('datePicker').addEventListener('change', loadAttendanceData);
...
document.getElementById('exportBtn').addEventListener('click', () => {
    ...
    .then(data => {
        const csvContent = 'data:text/csv;charset=utf-8,' +
            ...
            data.map(row => 
                `${row.student_id},${row.student_name},${row.time_in},${row.time_out},${row.status}`
            ).join('\n');
...
editForm.onsubmit = (e) => {
    ...
    fetch('attendance_actions.php', {
        ...
        body: `action=update&id=${updatedData.id}&name=${updatedData.name}&timeIn=${updatedData.timeIn}&timeOut=${updatedData.timeOut}&status=${updatedData.status}&date=${document.getElementById('datePicker').value}`
    })
    .then(response => response.json())
    .then(data => {
        if(data.success) {
            ...
            if (index !== -1) {
                attendanceData[index] = updatedData;
                renderTable(attendanceData);
                modal.style.display = 'none';
            }
        } else {
...
addForm.onsubmit = (e) => {
    ...
    fetch('attendance_actions.php', {
        ...
        body: `action=add&id=${newStudent.id}&name=${newStudent.name}&timeIn=${newStudent.timeIn}&timeOut=${newStudent.timeOut}&status=${newStudent.status}&date=${document.getElementById('datePicker').value}`
    })
    .then(response => response.json())
    .then(data => {
        if(data.success) {
